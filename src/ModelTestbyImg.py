...
path = r'dataRecord\WithoutV_LK1'
...
num_image = num_image_1+num_image_2
...
def create_dataset(path,batch_size,epoch_num):
    ...
    dataset = dataset.shuffle(buffer_size=600).batch(batch_size).repeat(epoch_num)  
    return dataset
...
#     #损失函数及优化算法
#     cross_entropy = tf.reduce_mean(-tf.reduce_sum(y_ * tf.log(y_conv), reduction_indices=[1]),name='cross_entropy') 
#     # cross_entropy1 = tf.nn.weighted_cross_entropy_with_logits(targets=y_, logits=y_conv,pos_weight=2.0)
#     # cross_entropy1 = tf.nn.sigmoid_cross_entropy_with_logits(labels=y_, logits=y_conv)
