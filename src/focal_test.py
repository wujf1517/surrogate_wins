...
num_image = num_image_1+num_image_2 # 总样本数
...
#     img_test, label_test = ReadMyownData.read_and_decode("123train.tfrecords")  # 123test.tfrecords
...
with tf.Session() as sess:
    ...
    # lower, upper = proportion_confint(correct, num_image, 0.05)
    # binomial confidence interval
    interval = 1.96 * sqrt( (correct_rate * (1 - correct_rate)) / num_image)
    print('%.3f' % interval)

    print('lower=%.3f, upper=%.3f' % (correct_rate-interval, correct_rate+interval))
    coord.request_stop()
    coord.join(threads)